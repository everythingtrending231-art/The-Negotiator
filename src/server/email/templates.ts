...
export function closureSummaryEmail(data: ClosureSummaryData) {
  ...
  return {
    ...
    html: `
      ...
      ${offerHtml}
      ${data.dealTicketUrl ? `<p><strong>Your deal ticket is ready</strong> — bring it${data.offerSummary?.businessName ? ` to ${escapeHtml(data.offerSummary.businessName)}` : ""} to complete your purchase: <a href="${data.dealTicketUrl}">${data.dealTicketUrl}</a> (a PDF copy with a QR code is attached to this email).</p>` : ""}
