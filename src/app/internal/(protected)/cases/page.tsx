...
import LogoutButton from "@/app/internal/(protected)/logout-button"
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <Link href="/internal/dev/outbox" className="text-sm text-slate-500 underline">
            Dev outbox
          </Link>
          <LogoutButton />
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Link href="/internal/cases">
          <Badge variant={!filterStatus ? "default" : "outline"}>All</Badge>
        </Link>
        {statuses.map((value) => (
          <Link key={value} href={`/internal/cases?status=${value}`}>
      ...
      <div className="bg-white rounded-xl shadow divide-y">
        ...
        {cases.map((negotiationCase) => (
          <Link
            ...
            href={`/internal/cases/${negotiationCase.id}`}
