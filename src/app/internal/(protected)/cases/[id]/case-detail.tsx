...
export default function CaseDetail(props: {
  ...
  const [actingAs, setActingAs] = useState(props.negotiationCase.assignedNegotiatorId ?? props.negotiators[0]?.id ?? "")
  ...
  async function assignToMe() {
    if (!actingAs) return
    await withBusy(() =>
      fetch(`/api/internal/cases/${props.negotiationCase.id}/assign`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ negotiatorId: actingAs }),
      }).then(() => undefined),
    )
  }
  ...
  async function updateStatus() {
    if (!actingAs) return
    await withBusy(() =>
      fetch(`/api/internal/cases/${props.negotiationCase.id}/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status, negotiatorId: actingAs }),
      }).then(() => undefined),
    )
  }

  async function addNote() {
    if (!actingAs || !note.trim()) return
    await withBusy(() =>
      fetch(`/api/internal/cases/${props.negotiationCase.id}/note`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ negotiatorId: actingAs, body: note }),
      }).then(() => setNote("")),
    )
  }

  async function sendMessage() {
    if (!actingAs || !message.trim()) return
    await withBusy(() =>
      fetch(`/api/internal/cases/${props.negotiationCase.id}/message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ negotiatorId: actingAs, body: message }),
      }).then(() => setMessage("")),
    )
  }
    ...
    <div className="max-w-3xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow p-6 space-y-4">
        <div className="flex items-center gap-3">
          <Label className="whitespace-nowrap">Acting as</Label>
        ...
        <p className="text-xs text-slate-400">
          Phase 1 has no per-negotiator login — every action below is attributed to whoever is selected here.
        </p>

        <div className="flex flex-wrap items-center gap-3 pt-2 border-t">
          <p className="text-sm text-slate-600">
            Assigned: {c.assignedNegotiatorName ?? "Unassigned"}
          </p>
          <Button size="sm" disabled={busy || !actingAs} onClick={assignToMe}>
            Assign to selected negotiator
          </Button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t">
          ...
          <Button size="sm" disabled={busy || !actingAs} onClick={updateStatus}>
            Update status
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 space-y-4">
        ...
        <Button size="sm" disabled={busy || !actingAs || !note.trim()} onClick={addNote}>
          Add note
        </Button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6 space-y-4">
        ...
        />
        <Button size="sm" disabled={busy || !actingAs || !message.trim()} onClick={sendMessage}>
          Send to customer
        </Button>
      </div>
...
function OfferForm(props: {
  ...
  async function submit() {
    if (!props.negotiatorId || !businessId || !finalPrice || !includedGoods.trim()) {
      setError("Business, final price, and included goods are required.")
      return
    }
    setSubmitting(true)
    setError(null)
    const res = await fetch(`/api/internal/cases/${props.caseId}/offer`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
